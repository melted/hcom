...
DLLEXPORT(char *) hresultString(HRESULT hr)
{
    ...
    FormatMessageA(FORMAT_MESSAGE_FROM_SYSTEM, NULL, hr, MAKELANGID(LANG_NEUTRAL, SUBLANG_NEUTRAL), msgbuf + len, 256,
                   NULL);
...
{
    if (guid)
        return CLSIDFromString(guidStr, guid);
    else
...
HRESULT primProgIDFromCLSID(const CLSID *clsid, WCHAR **clsidStr)
{
    if (!clsid)
        clsid = &IID_NULL;
    if (clsidStr)
        return ProgIDFromCLSID(clsid, clsidStr);
    else
        return E_POINTER;
}
...
HRESULT primQI(/*[in]*/ void *methPtr, /*[in]*/ void *iptr, /*[in]*/ void *riid, /*[in]*/ void **ppv)
{
...
unsigned int primAddRef(/*[in]*/ void *methPtr, /*[in]*/ void *iptr)
{
...
unsigned int primRelease(/*[in]*/ void *methPtr, /*[in]*/ void *iptr)
{
...
HRESULT primLoadRegTypeLib(GUID *rguid, short wMaj, short wMin, LCID lcid, void **ppv)
{
    HRESULT hr;
...
BSTR primQueryPathOfRegTypeLib(GUID *rguid, unsigned short maj, unsigned short min)
{
    BSTR bs;
    HRESULT hr;
...
char *getModuleFileName(HANDLE hMod)
{
...
int bstrLen(/*[in]*/ BSTR bstr)
{
    ...
    return WideCharToMultiByte(CP_ACP, 0, bstr, SysStringLen(bstr), NULL, 0, NULL, NULL);
}

HRESULT
bstrToStringLen(BSTR bstr, int len, char *p)
{
    int i;
    if (!p)
    {
    ...
    i = WideCharToMultiByte(CP_ACP, 0, bstr, SysStringLen(bstr), p, len, NULL, NULL);
    if (i == 0)
    {
        return E_FAIL;
    }
 ...
 */
void primGetVersionInfo(/*[out]*/ unsigned long *maj, /*[out]*/ unsigned long *min, /*[out]*/ unsigned long *pid)
{
    OSVERSIONINFO oe;
    BOOL b;
...
void releaseIUnknown__(void *ip)
{
    DWORD d;
#ifdef DEBUG
