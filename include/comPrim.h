...
extern void *STDCALL CoTaskMemAlloc(unsigned int size);

void STDCALL CoTaskMemFree(LPVOID pv);

extern HRESULT STDCALL GetActiveObject(CLSID *clsid, void *reserved, IUnknown **ppunk);
...
#if __W32API_MAJOR_VERSION == 1
extern HRESULT STDCALL CreateTypeLib(int syskind, LPCOLESTR szFile, void **ppv);

extern HRESULT STDCALL CoCreateGuid(GUID *pguid);

extern HRESULT STDCALL CreateTypeLib2(int syskind, LPCOLESTR szFile, void **ppv);
#endif
...
#if __W32API_MAJOR_VERSION == 1
BOOL STDCALL IsEqualGUID(const GUID *g1, const GUID *g2);

HRESULT
STDCALL
LoadTypeLib(LPOLESTR lpsz, IUnknown **ppv);

HRESULT
STDCALL
LoadTypeLibEx(LPOLESTR lpsz, int kind, IUnknown **ppv);

HRESULT
STDCALL
LoadRegTypeLib(GUID *rguid, WORD wVerMajor, WORD wVerMinor, LCID lcid, IUnknown **ppv);
#endif

extern HRESULT primLoadRegTypeLib(GUID *rguid, short wMaj, short wMin, LCID lcid, void **ppv);

#if __W32API_MAJOR_VERSION == 1
extern HRESULT STDCALL QueryPathOfRegTypeLib(GUID *rguid, unsigned short maj, unsigned short min, LCID lcid,
                                             BSTR *pbstr);

#endif

extern BSTR primQueryPathOfRegTypeLib(GUID *rguid, unsigned short maj, unsigned short min);

extern char *getModuleFileName(HANDLE hMod);

#if __W32API_MAJOR_VERSION == 1
extern DWORD WINAPI GetModuleFileNameA(HINSTANCE hMod, LPSTR lpFileName, DWORD size);

extern LCID WINAPI GetUserDefaultLCID();

extern INT STDCALL SystemTimeToVariantTime(void *lpSystemTime, double *pvarTime);
#endif
